...
// Debug mode: false = do not auto-print alternating pages in Serial Monitor.
const bool ENABLE_PAGE_CAROUSEL = false;
...
printPersonalityState(currentPersonality);
  ...
  if (success && ENABLE_PAGE_CAROUSEL) {
    showCurrentPage();
  }
}
...
void loop() {
  ...
  if (now - lastWeatherRefresh >= WEATHER_REFRESH_INTERVAL_MS) {
    ...
    if (success) {
      ...
      // 调试 TinyML 时不自动显示页面，避免串口被刷乱
      if (ENABLE_PAGE_CAROUSEL) {
        showCurrentPage();
      }
    } else {
